...
pub fn run_window_loop(scene: SceneGraph) {
    ...
    if let Some(rect_cmds) = planned_batches.get(0) {
        let owned_cmds: Vec<_> = rect_cmds.iter().map(|&c| match c {
            crate::renderer::commands::command_builder::RenderCommand::SolidRect { rect, rgba } => {
                crate::renderer::commands::command_builder::RenderCommand::SolidRect { rect: *rect, rgba: *rgba }
            },
            crate::renderer::commands::command_builder::RenderCommand::Text { text, rect, rgba, font_size } => {
                crate::renderer::commands::command_builder::RenderCommand::Text { text: text.clone(), rect: *rect, rgba: *rgba, font_size: *font_size }
            },
        }).collect();
